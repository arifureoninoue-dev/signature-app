...
BLOB_READ_WRITE_TOKEN = os.environ.get("BLOB_READ_WRITE_TOKEN") # Blobのトークンを環境変数から取得
...
def sign():
    ...
    if signature_data_url:
        ...
        try:
            ...
            # --- Vercel Blobへのアップロード処理 (requestsを使用) ---
            headers = {
...
def download_page():
    ...
    return render_template('download.html', signature_url=signature_url, explainers=list(set(available_explainers)))
...
def generate_pdf():
    ...
    try:
        # --- Vercel Blobからのダウンロード処理 (requestsを使用) ---
    ...
    try:
        ...
        # --- Vercel BlobへのPDFアップロード処理 (requestsを使用) ---
        headers = {
            'Authorization': f'Bearer {BLOB_READ_WRITE_TOKEN}',
